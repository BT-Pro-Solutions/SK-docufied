# Explanation
This C# code demonstrates various ways to use the `ITextSearch` interface from Microsoft Semantic Kernel for Retrieval Augmented Generation (RAG). The file provides several unit test examples (marked with `[Fact]`) showing how to integrate external web search results—such as from Bing or Google—into prompts sent to large language models via Semantic Kernel. Examples illustrate:

- Basic RAG workflow using Bing or Google search as contextual grounding.
- Including citations and timestamps from search results in responses.
- Restricting searches to specific sites (e.g., Microsoft Developer Blogs).
- Customizing search parameters and prompts.
- Retrieving and embedding the full content (HTML converted to plain text) of web pages as search results.

The code also includes a helper class `TextSearchWithFullValues` that wraps an `ITextSearch` implementation to fetch and convert full web pages to plain text for more comprehensive grounding.

Below is the complete code file as provided.
