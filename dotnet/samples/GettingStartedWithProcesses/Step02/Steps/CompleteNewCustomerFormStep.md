# Explanation
This C# code defines a step in a conversational AI process designed to help a user complete a new customer account form. The `CompleteNewCustomerFormStep` class manages interactions to collect all necessary information for opening a new account, such as the user's first name, last name, date of birth, state of residence, phone number, user ID, and email address. It uses a kernel (likely an AI-driven framework) to guide the conversation, process user responses, and update the form state accordingly.

Key features include:
- A welcome message to start the form-filling process.
- A system prompt that guides the AI assistant in helping the user provide all required form fields.
- Functions to handle individual user data entries and update the internal form state.
- Tracking of the conversation history between the user and the assistant.
- Emitting events when the form is complete or when more information is needed.

The `NewCustomerFormState` class holds the current form data and the conversation history.
