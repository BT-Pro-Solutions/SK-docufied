# Explanation
This C# program provides a console-based interface for interacting with a variety of large language models available through AWS Bedrock and other providers (Anthropic, Cohere, Meta, Mistral, Amazon Titan, etc.) using the Microsoft Semantic Kernel SDK. The user can choose among four options: Chat Completion, Text Generation, Streamed Chat Completion, or Streamed Text Generation. 

The code dynamically lists available models for each modality (chat or text, streaming or not), allows the user to select a model, and processes user input accordingly. Chat and text history are maintained where applicable, and responses along with some metadata (usage, etc.) are displayed. The `ModelDefinition` struct encodes metadata about each available model, such as supported modalities and streaming capability.

The program is modular and demonstrates how to use Semantic Kernel services (chat completion and text generation) with Bedrock models in both standard and streaming modes.
