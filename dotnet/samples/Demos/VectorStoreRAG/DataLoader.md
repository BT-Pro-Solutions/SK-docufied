...
This C# code defines an internal sealed class `DataLoader<TKey>` for the VectorStoreRAG namespace, designed to load textual and image content from a PDF file into a vector store. It reads both text blocks and images from the PDF, converts images to text using a chat completion service, generates embeddings for each text snippet, and stores them in a vector store collection. The loader operates in batches, supports delays between batches, retries on certain errors, and includes cancellation support. Key dependencies include PDF processing (`UglyToad.PdfPig`), Microsoft Semantic Kernel for vector/embedding operations, and associated services for uniqueness and chat/image-to-text processing.
