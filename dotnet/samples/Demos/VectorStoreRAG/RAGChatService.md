...
The `RAGChatService` class is a hosted service in a .NET application that manages the workflow of a Retrieval-Augmented Generation (RAG) chat assistant. This service:

- Optionally loads configured PDF files into a vector store for semantic search, depending on configuration.
- Maintains a main chat loop which:
    - Waits for the PDF data load to complete successfully.
    - Prompts the user for questions to be answered using the content of the loaded PDFs.
    - Executes a semantic search using a vector store for relevant information to augment responses.
    - Constructs an LLM prompt that includes citations from the PDF content and streams the response back to the user.
    - Exits gracefully on user request or application cancellation.

The class uses dependency injection to receive its data loader, search, kernel (LLM access), configuration, and a cancellation token for orderly shutdown. It leverages the Semantic Kernel SDK from Microsoft for AI prompt orchestration and data management.
