# Explanation
This C# file defines a static class `CommonExtensions` that provides extension methods to simplify adding common services and configuration for .NET applications built with ASP.NET Core. The extensions include configuring OpenTelemetry for distributed tracing, metrics, and logging; adding default health checks for liveness; setting up service discovery and HTTP client resilience defaults; and mapping health check endpoints. These methods are designed to streamline configuring these features with sensible defaults, helping developers quickly integrate observability, resilience, and health monitoring into their applications.
