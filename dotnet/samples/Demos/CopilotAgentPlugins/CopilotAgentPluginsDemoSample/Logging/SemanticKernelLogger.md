# Explanation
This C# class, `SemanticKernelLogger`, implements the `ILogger` interface to provide custom logging functionality. It processes and formats log messages to enhance readability, especially when dealing with JSON content. Leveraging the Spectre.Console library, it displays JSON data within styled panels in the console, highlighting sections such as "Rendered prompt," "Function result," "Function arguments," and "Plan result." The logger attempts to parse and prettify JSON segments and nested JSON objects within log messages. If a message does not match any special patterns, it prints the log level and the message in green text.
