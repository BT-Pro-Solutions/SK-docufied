...
The `DemoCommand.cs` file implements an interactive command-line demo using Spectre.Console and Microsoft's Semantic Kernel. The command allows users to select between different AI model backends (Azure OpenAI, OpenAI, Ollama), load or unload Copilot Agent plugins (API plugins with OpenAPI manifest integration), view plugin manifests, execute user goals (natural language prompts), and inspect loaded plugins with their functions and parameters.

Key features in the code:

- Dynamic selection between AI kernel backends, each initialized according to configuration found in `appsettings.Development.json`.
- Loading and unloading of plugins from a directory of Copilot Agent plugins, each represented by an OpenAPI manifest and supporting authentication flows (API key or Bearer).
- Console-driven menu for all supported actions, with presentation using Spectre.Console UI components.
- Listing of loaded plugins, including their available functions and function parameters.
- Specialized logic to filter out certain properties in OpenAPI schemas for strict/compact function calls.
- Integration of logging as an optional command switch.
- Encapsulated classes and functions for extensibility (e.g., adding more plugin types or authentication strategies).

This code is meant for demonstration and concept validation when building extensible AI agent applications relying on Microsoft's Semantic Kernel and pluggable API skills.
