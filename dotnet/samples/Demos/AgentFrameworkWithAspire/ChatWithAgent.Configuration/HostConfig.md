# Explanation
This C# code defines a `HostConfig` class designed to load and hold configuration settings related to AI services and vector stores for an application that likely interacts with Azure OpenAI and OpenAI services. The class reads configuration sections from a provided `ConfigurationManager` instance, binding them to strongly-typed configuration objects for different AI services (chat and embeddings) as well as vector stores and retrieval-augmented generation (RAG) settings. The class provides properties to access these configurations after they have been loaded.
