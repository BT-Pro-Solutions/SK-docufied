# Explanation
This C# class `AgentCompletionsApiClient` provides a client for interacting with an agent completions API. It sends prompts along with chat history to the API and receives streaming completion results asynchronously. The class manages chat history locally and supports streaming the response content back to the caller piece by piece, making it suitable for real-time UI updates or progressive output scenarios.
