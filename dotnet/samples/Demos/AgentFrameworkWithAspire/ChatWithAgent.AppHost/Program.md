# Explanation
This C# program sets up and runs a distributed application using a builder pattern, configuring AI services and vector stores based on the provided host configuration. It integrates Azure OpenAI and OpenAI services for chat and embedding functionalities, sets up vector stores like Azure AI Search, and configures both an API service and a web frontend with proper references and environment settings. The application builds all components and runs the web frontend dependent on the API service's readiness.
...
static List<IResourceBuilder<IResourceWithConnectionString>> AddAIServices(IDistributedApplicationBuilder builder, HostConfig config)
{
    ...
    return new List<IResourceBuilder<IResourceWithConnectionString>> { chatResource, embeddingsResource };
}
