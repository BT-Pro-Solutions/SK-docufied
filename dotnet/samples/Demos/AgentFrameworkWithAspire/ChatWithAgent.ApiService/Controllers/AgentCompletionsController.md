# Explanation
This C# ASP.NET Core controller class, `AgentCompletionsController`, handles HTTP POST requests to provide conversational completions via a chat agent. It supports both streaming and non-streaming completions from a chat-based AI agent (`ChatCompletionAgent`). The controller accepts requests containing a user prompt and chat history, validates the input, runs the agent to generate a response, and returns the response either as a single collection of messages or as a stream of partial messages. It ensures system messages are excluded from client-provided chat history for correct operation.
