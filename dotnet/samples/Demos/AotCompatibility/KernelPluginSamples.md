# Explanation
This C# code file demonstrates several sample methods for creating, importing, and adding kernel plugins in the context of Ahead-Of-Time (AOT) .NET applications using the Microsoft Semantic Kernel. The main focus is on handling plugins that use custom types (`Location` and `Weather`) which require explicit type information via `JsonSerializerOptions` and custom `JsonSerializerContext` instances to ensure AOT compatibility. The samples show how to:
- Create a kernel plugin from a type and invoke its function.
- Import a plugin into the kernel's plugin collection from a type and invoke its function.
- Add a plugin to the kernel's plugin collection from a type and invoke its function.

Each operation highlights the necessity of configuring serialization options correctly to work with strongly-typed plugin arguments and results in AOT scenarios.
