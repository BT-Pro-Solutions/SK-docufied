# Explanation
This C# class, `MailboxUtils`, provides utility methods to work with mailbox content, specifically summarizing unread emails using Microsoft's MCP (Model Context Protocol) server's sampling feature. It creates sample email instances with attachments, converts them into a list of messages suitable for the MCP sampling request, sends this request to the MCP server to generate a summary, and returns the summarized text. The class includes helper methods to transform email information into `SamplingMessage` objects and a nested `Email` class representing an email with sender, subject, body, and attachments.
