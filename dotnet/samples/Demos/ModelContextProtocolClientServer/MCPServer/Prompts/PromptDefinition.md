# Explanation
This C# class defines a `PromptDefinition` that represents a prompt configuration and its handler for rendering the prompt in the context of Microsoft Semantic Kernel (SK) integrated with Model Context Protocol (MCP). It allows creation of prompt definitions from JSON templates, using Handlebars templating for rendering. The class encapsulates the prompt's metadata and implements an asynchronous handler to produce the rendered prompt string as a result based on input parameters. It also connects with the dependency injection system to get or use a provided SK kernel instance.
