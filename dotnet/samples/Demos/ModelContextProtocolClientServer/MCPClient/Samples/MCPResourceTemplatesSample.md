# Explanation
This C# code sample demonstrates how to use Model Context Protocol (MCP) resource templates in combination with the Microsoft Semantic Kernel. It shows how to create an MCP client, list available resource templates from the MCP server, read records relevant to a given prompt via an MCP resource template, and then use those records as part of a chat history to prompt an AI chat completion model. The example specifically prompts the model with the question "What is the Semantic Kernel?" using relevant vector-store records fetched via MCP, and then outputs the AI model's response.
