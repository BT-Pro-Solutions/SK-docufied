# Explanation
This C# code defines an abstract base class `BaseSample` used for creating clients that connect to an MCP (Model Context Protocol) server. It provides utility methods for:
- Creating an MCP client linked to a server executable via standard I/O transport.
- Optionally adding a custom sampling request handler integrated with the Microsoft Semantic Kernel.
- Creating a configured Semantic Kernel instance that uses OpenAI chat completion services.
- Displaying available MCP tools in the console.
- Locating the MCP server executable based on the build configuration.

The class encapsulates setup and helper functionality typically used in MCP client samples, assisting developers in connecting and interacting with MCP servers and related AI services.
