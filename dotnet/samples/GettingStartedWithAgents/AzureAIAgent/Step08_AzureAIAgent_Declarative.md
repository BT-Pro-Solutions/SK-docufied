...
This code sample demonstrates how to declaratively create and use Azure AI Agents in C# using the Microsoft Semantic Kernel and Azure AI SDKs. The `Step08_AzureAIAgent_Declarative` class contains a series of test cases (annotated with `[Fact]`) illustrating different ways to define and instantiate Azure AI Agents from YAML configurations.

Key scenarios shown include:
- Creating agents with specific models and instructions using YAML text.
- Adding tool integrations such as code interpreter, function calls, Bing search grounding, file search, and integrating with OpenAPI (both JSON and YAML specs).
- Supplying kernel plugins to augment agent functionality.
- Parameterizing agent definitions using environment variables or configuration bindings.
- Invoking the agents with natural language prompts and collecting responses.
- Configuring agent templates supporting variable inputs and outputs.
- Properly disposing of agents and associated resources after use.

The code leverages the Azure.AI.Projects, Microsoft.SemanticKernel, and related libraries for composing agents, building kernel pipelines, and managing dependencies. This sample is useful for developers looking to modularly configure and extend generative AI agents (such as chatbots or copilots) in enterprise .NET applications.
