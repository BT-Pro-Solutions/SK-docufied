# Explanation
This code demonstrates how to use Azure AI Agents in a group chat setting using the Microsoft Semantic Kernel. Specifically, it showcases the creation of an `AgentGroupChat` involving two agents—an art director (reviewer) and a copywriter—with individualized instructions and roles. The chat process employs a custom `ApprovalTerminationStrategy` that ends the conversation when the art director's response includes the word "approve," or after a maximum number of turns. This example is structured as an xUnit test, sending an initial user prompt and iterating through chat responses, displaying each message until the conversation completes or hits the specified limit.
