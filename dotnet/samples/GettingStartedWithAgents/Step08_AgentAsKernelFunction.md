...
This C# code demonstrates the use of the Microsoft Semantic Kernel framework to create and use AI agents—specifically, agents functioning as kernel functions within a conversational or chat-based context. The code shows how to define agents such as "SalesAssistant" and "RefundAgent", configure them with appropriate plugins for order and refund functionalities, and interact with them via chat messages. It further demonstrates agent composition by allowing delegation between agents (as shown in the "MultipleAgents" example). Tests showcase orchestration of chat-based tasks, including agent responses, plugin invocation, and conversation thread management. Supporting classes implement plugin logic and tracing for function invocation during testing.
