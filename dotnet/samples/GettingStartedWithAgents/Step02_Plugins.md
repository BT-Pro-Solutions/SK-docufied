# Explanation
This C# code demonstrates how to use the Semantic Kernel SDK to create and interact with chat completion agents that leverage plugins. The `Step02_Plugins` test class contains several factual (unit) tests showing how to set up agents with plugins for menu and widget interactions, how to auto-invoke plugins using template execution settings, and how to manage conversation history. The code illustrates initializing agents, providing instructions, adding plugins programmatically, and invoking the agent in a conversational thread while outputting the conversation.
