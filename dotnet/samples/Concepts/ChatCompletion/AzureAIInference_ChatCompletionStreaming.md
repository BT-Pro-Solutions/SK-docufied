...
This file, `AzureAIInference_ChatCompletionStreaming.cs`, demonstrates how to utilize streaming chat completion functionalities with Azure AI Inference models, as well as GitHub models compatible with OpenAI's API, within a .NET application. The class contains various test examples that:

- Show how to stream AI-generated chat messages in real time using Azure's chat completion service.
- Illustrate streaming chat responses both directly (using client APIs) and via a semantic kernel abstraction.
- Provide mechanisms for capturing and displaying incremental assistant responses, showing both direct content and individual message items.
- Build sample chat histories, output stream results, and demonstrate interaction sequences with the AI model.
  
The code leverages Azure.AI.Inference SDK, the Microsoft Semantic Kernel, and makes extensive use of async/await for real-time, non-blocking streaming of chat messages. It is structured for use within a unit testing framework (e.g., xUnit, using `[Fact]` attributes).
