...
This C# code demonstrates how to use Google Gemini and Vertex AI for chat completion tasks using the Microsoft Semantic Kernel framework. It contains a test class, `Google_GeminiChatCompletion`, which provides two main examples:

1. **Google Gemini Chat Completion**: Shows how to use Google's Gemini model via API key to perform chat completions.
2. **Vertex AI Gemini Chat Completion**: Illustrates how to connect to Vertex AI's Gemini model using a bearer token, including comments on secure bearer token handling and token refresh strategies suitable for production systems.

Both examples build a `Kernel` with the corresponding chat completion service, construct a chat history with multi-turn dialogue, and display the assistant's replies. The implementation guides the user through managing credentials, token provision, and proper use of the Semantic Kernel's chat interfaces.
