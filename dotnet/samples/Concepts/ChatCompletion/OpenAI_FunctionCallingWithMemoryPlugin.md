# Explanation
This C# code demonstrates how to use the `TextMemoryPlugin` within the Semantic Kernel SDK to retrieve relevant information from a memory store using OpenAI's chat completion and text embedding APIs. The code sets up a Semantic Kernel instance, adds support for chat and embeddings via OpenAI models, initializes an in-memory text storage, and populates it with sample knowledge about Semantic Kernel. A "Memory" plugin is registered to the kernel and exposes a function that retrieves relevant entries from the memory to answer user questions. The test case shows how a chat prompt can automatically use the memory function to answer the question "What is Semantic Kernel?", logging the function invocation during the process.
