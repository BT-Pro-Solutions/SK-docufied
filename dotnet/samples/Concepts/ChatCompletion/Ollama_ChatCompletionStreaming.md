...
This C# code file provides several examples demonstrating how to use chat completion streaming with the Ollama API in various scenarios. It covers the use of the OllamaSharp library for making chat requests, handling streaming responses, and integrating chat completion streaming with Microsoft Semantic Kernel (`Kernel`). The examples focus on:
- Basic chat completion streaming with a simulated librarian expert bot.
- Retrieving internal library information (like streaming chunk details).
- Using templated chat prompts with the Semantic Kernel and streaming the results.
- Accessing streaming responses via text content and internal structures.
- Demonstrating potential version brittleness when accessing underlying stream objects.

The code is organized as a set of test methods (using xUnit's `[Fact]` attribute), suitable for illustrating API capabilities and usage patterns.
