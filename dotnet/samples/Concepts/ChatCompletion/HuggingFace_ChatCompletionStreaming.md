# Explanation
This C# code demonstrates how to use the Hugging Face Text Generation Inference (TGI) API for chat completion within the Microsoft Semantic Kernel framework. The class provides two examples: one for using the `IChatCompletionService` directly with a `ChatHistory` and another for invoking a streaming chat completion using a custom prompt within the Kernel. The code assumes you've set up a local Hugging Face TGI server (using Docker) as described in the official Hugging Face documentation. The code also shows how to set model parameters and handle streaming output for chat-based tasks.
