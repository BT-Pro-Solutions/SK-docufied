...
This code file demonstrates how to use Azure OpenAI Chat Completion with data sources in a .NET environment, specifically leveraging Azure AI Search (formerly Cognitive Search) as an auxiliary data source to enrich chat completions. The examples illustrate multiple scenarios:
- Basic chat completion with Azure OpenAI and an external data source.
- Maintaining chat history and performing chat completion in streaming mode.
- Using Semantic Kernel's function calling and custom plugins to extend chat capabilities.
- A robust retry mechanism that automatically switches between using the Azure data source and function-calling based on the quality/completeness of the response.
- Citation extraction and display from the returned responses.

It includes setup notes for uploading example content to Azure Blob Storage and configuring secrets for Azure AI Search. Additionally, helper classes and methods are provided to handle citations, manage chat history, implement retry logic, parse model results, and simulate a data plugin.
