# Explanation
This C# code demonstrates how to use the Microsoft Semantic Kernel with OpenAI (and MistralAI) chat completion services to automatically invoke kernel functions in response to chat prompts. It sets up test cases (presumably using xUnit, as indicated by the [Fact] attributes) for various plugins (`WeatherPlugin`, `HolidayPlugin`, and `LightPlugin`), showcasing automatic function invocation, handling chat history, and working with complex parameter types. The code also provides examples of plugin classes with functions decorated for semantic kernel use, a reusable mechanism for building kernels with different plugins, and a demonstration of invocation filtering for logging purposes.
