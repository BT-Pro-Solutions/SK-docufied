...
This C# code demonstrates various ways to perform chat completions using the OpenAI API within Microsoft's Semantic Kernel framework. The `OpenAI_ChatCompletion` class (with xUnit test support) shows multiple sample usage patterns:

- Using the `IChatCompletionService` interface directly with a `ChatHistory` to send and receive conversational messages in sequence.
- Accessing the lower-level OpenAI SDK response objects for detailed inspection via "breaking glass" scenarios by using `InnerContent`.
- Utilizing the `Kernel` and prompt syntax to interact with chat completions and managing conversation flow.
- Storing chat completion requests with metadata, useful for tracking or filtering in OpenAI's dashboard and enabling model evaluation or distillation.
- Outputting detailed response information, including token-level log-probabilities for both responses and refusal reasons, if provided.
- The class provides utility methods to output the last messages and to print deep debugging information about the OpenAI response's inner structure and statistics.

The code is intended as documentation/examples and is not for production, with several advanced/experimental uses shown that rely on internal representations ('breaking glass'), so one should be cautious if the OpenAI SDK changes.
