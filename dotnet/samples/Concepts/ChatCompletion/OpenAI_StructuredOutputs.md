# Explanation
This C# code demonstrates how to use the Structured Outputs feature in the OpenAI (and Azure OpenAI) API via the Microsoft Semantic Kernel SDK. Structured Outputs allow developers to specify a desired JSON schema (or a .NET type that maps to one), constraining the model's output to a precise structure, thereby reducing the risk of hallucinations and simplifying parsing.

The code provides test examples for:
- Using a custom JSON schema for responses (`ChatResponseFormat`).
- Specifying a .NET type as the desired response format (letting the SDK generate the schema automatically).
- Combining Structured Outputs with function calling—allowing AI to transform and categorize email data.
- Using Azure OpenAI endpoints and chat completion API with Structured Outputs (requires recent versions/APIs).
- Composing plugins/functions from prompt directories and YAML configuration, injecting structured outputs into template-based functions.

Private helper structs/classes are used for strong typing and output formatting (e.g., `MovieResult`, `Movie`, `EmailResult`, `Email`). The `EmailPlugin` class simulates retrieval of emails for categorization. Each scenario illustrates deserialization of model output into these strong types, demonstrating reliable parsing thanks to the structure guarantee.

**Important notes:**
- Structured Outputs are only available on certain recent models (e.g., GPT-4o) and latest API versions.
- Not all JSON Schema keywords are supported (see code comments for limitations).
