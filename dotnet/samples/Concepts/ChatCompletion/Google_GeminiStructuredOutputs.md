...
This C# file provides sample implementations demonstrating the use of "Structured Outputs" with Google Gemini (Vertex AI) chat completion within the Microsoft Semantic Kernel framework. Structured Outputs enforce that responses from the model follow a defined JSON schema, enhancing type safety, reducing hallucinations, and making prompt engineering simpler.

Key scenarios illustrated:

- **Structured Outputs using Type in Execution Settings:** Shows how to specify the expected response format using a C# type (e.g., `User` or `MovieResult`) so responses are in the specified JSON shape.
- **Structured Outputs with Function Calling:** Combines structured outputs with function calling, using plugins and strong typing to structure and transform data (e.g., categorizing email bodies).
- **Structured Outputs with Functions from Prompts/YAML:** Demonstrates how to use pre-defined SK functions (built from prompt configuration files or YAML) in conjunction with structured outputs for typed results.
- **Helper Methods and Data Models:** Contains strongly typed models (`User`, `MovieResult`, `Movie`, `EmailResult`, `Email`) and helper functions to output results, plus plugin demonstrations (e.g., simulated RAG scenarios returning collections of data).

The tests dynamically initialize the kernel to use either Google AI or Vertex AI backend, with credentials, and demonstrate good patterns for bearer token management for Vertex AI. The code is intended for automated testing and documentation sample purposes.
