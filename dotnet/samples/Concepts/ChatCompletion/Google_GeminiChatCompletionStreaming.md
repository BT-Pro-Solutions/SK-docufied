# Explanation
This C# code demonstrates how to perform streaming chat completion using Google Gemini models through both the Vertex AI and Google AI APIs. It provides example test cases for interacting with Google's Generative AI chat services—one using an API key and the other using OAuth2 Bearer tokens—with the Microsoft Semantic Kernel. The methods showcase building the kernel, authenticating with appropriate credentials, processing chat histories, and streaming chat completions while outputting messages to the console. The code also illustrates the recommended way of acquiring and refreshing bearer tokens securely.
