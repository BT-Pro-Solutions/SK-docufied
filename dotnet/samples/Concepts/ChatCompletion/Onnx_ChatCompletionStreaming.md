...
This C# code demonstrates how to perform streaming chat completion using ONNX GenAI models through the Microsoft.SemanticKernel library. It includes different approaches to streaming chat responses:

1. **StreamChatAsync**  
   Uses `OnnxRuntimeGenAIChatCompletionService` directly to stream chat completions for a conversational chat history between a user and an AI librarian assistant. It outputs responses as they arrive.

2. **StreamChatPromptAsync**  
   Uses the Semantic Kernel abstraction for streaming completions by constructing a chat prompt and running it through a configured kernel with ONNX GenAI chat completion added as a service.

3. **StreamTextFromChatAsync**  
   Demonstrates how to stream text content from AI responses, showing how to access the returned text from both high-level (`Content` property) and detailed (`Items` collection) perspectives.

It provides helper methods to output streamed messages, clean up kernel services, and uses `TestConfiguration.Onnx.ModelId` and `TestConfiguration.Onnx.ModelPath` for model configuration. The examples expect a local ONNX model directory and require configuration of secret values for the model parameters.

The code is organized as an xUnit test class with `[Fact]` annotations, suitable for automated tests or interactive exploration.
