...
This C# code file provides a set of example unit tests demonstrating how to use Azure OpenAI's streaming chat completion capabilities using the Microsoft Semantic Kernel SDK. The class `AzureOpenAI_ChatCompletionStreaming`, intended for xUnit test execution, includes methods that show:

- How to create and stream chat messages using the Azure OpenAI API.
- How to handle and display streaming text updates for chat completions.
- How to handle streaming function call content for scenarios involving function calling (where the LLM may determine a function should be invoked during a chat).
- How to use chat history with user and assistant messages.
- How to set up a test environment for Azure OpenAI and use prompt execution settings related to function calling.

Each example demonstrates practical usage of the SDK for real-time streaming scenarios, outputting results to the console for demonstration purposes.
