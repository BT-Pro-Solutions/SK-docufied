...
This C# code file defines the `SafeChatPrompts` class, which demonstrates various ways to handle prompt templates safely or unsafely when working with chat completions using the Microsoft Semantic Kernel framework. The class focuses on showing how to trust or restrict content in chat prompts, including:
- Allowing or restricting input variables, functions, or template sections that may contain unsafe or potentially malicious content.
- Examples include scenarios where content is marked as trusted, HTML-encoded, inserted as CDATA, or where variables/functions may inject unexpected markup.

The code relies on plugin functions, argument passing, and prompt configuration to illustrate how prompt sanitization works, as well as how to override it through `AllowDangerouslySetContent`. Utility methods are supplied to render prompts and invoke the kernel for actual completion results. Each method is a separate test styled as an [Fact] method for use with an xUnit-style testing framework.
