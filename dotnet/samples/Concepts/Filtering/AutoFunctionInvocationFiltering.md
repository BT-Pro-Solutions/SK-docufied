...
This C# code demonstrates how to use the `IAutoFunctionInvocationFilter` in the Microsoft Semantic Kernel to intercept and customize function invocation within an AI kernel using OpenAI models. Two test cases are shown:

1. `AutoFunctionInvocationFilterAsync` – Illustrates how to plug a custom auto function invocation filter that logs invocation details and can override function results or terminate function execution.
2. `GetFunctionCallsWithFilterAsync` – Shows how to use a filter to retrieve and log all function calls per LLM (Large Language Model) request, using a streaming approach.

The code also includes two sample filter implementations:
- `AutoFunctionInvocationFilter`: Demonstrates accessing the invocation context, logging information, overriding results, and terminating invocation.
- `FunctionCallsFilter`: Logs all function calls per LLM request.

This is helpful when building advanced AI agents or orchestrators that require fine-grained control over kernel function routing, logging, and result manipulation.
