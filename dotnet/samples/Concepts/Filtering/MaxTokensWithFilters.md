...
This C# code demonstrates how to manage and limit the total number of tokens used in an AI-driven function-calling workflow using Microsoft's Semantic Kernel and OpenAI chat models. The example highlights the difference between specifying a maximum token count for individual requests versus enforcing a stricter token usage across an entire multi-step chat/completion process, such as auto function calling where multiple AI requests may occur sequentially.

A custom filter (`MaxTokensFilter`) is implemented to dynamically adjust the maximum tokens allowed for subsequent requests by subtracting previously used tokens, ensuring the total does not exceed a global max (`MaxTokens`). The code includes a demonstration with and without the filter, showing the reduction in overall token consumption when the filter is applied.

Auxiliary classes and methods demonstrate how to plug user-defined functions (`MoviePlugin`) into the kernel and how to extract and tally output token usage from chat history.
