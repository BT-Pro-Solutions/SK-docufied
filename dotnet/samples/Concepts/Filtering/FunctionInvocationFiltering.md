...
This C# file demonstrates how to use function and prompt filters with Microsoft's Semantic Kernel. Filters (implementing `IFunctionInvocationFilter`) allow developers to intercept, modify, or handle function invocations, both for standard and streaming scenarios. The examples include:

- Adding filters through dependency injection.
- Overriding results in non-streaming and streaming modes.
- Exception handling within filters for both standard and streaming invocations.
- Logging filter invocation order using test output helpers.
- Combining both streaming and non-streaming result overrides.

The code uses xUnit for test methods and includes several inner classes each showing a filtering scenario, allowing comprehensive end-to-end testing and demonstration of Semantic Kernel's filtering capabilities.
