...
This C# code demonstrates how to add custom telemetry and logging to the Microsoft Semantic Kernel using filters. The class `TelemetryWithFilters` sets up a Semantic Kernel with OpenAI GPT-4 chat completion, adds sample functions (current time and weather for a city), and attaches several logging filters via dependency injection. These filters log detailed diagnostic information during kernel execution, including function invocations, arguments, prompt rendering, automatic function calling, results, token usage, and execution duration. The example is designed for extensibility: you can add or customize logging according to your needs, especially for custom connectors or advanced monitoring. Test cases illustrate both streaming and standard prompt invocation, with detailed logs captured under a transaction scope.
