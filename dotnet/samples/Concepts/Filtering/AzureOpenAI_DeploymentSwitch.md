# Explanation
This C# code demonstrates how to dynamically switch between different Azure OpenAI deployments based on which functions are being called in a Semantic Kernel application. By integrating multiple Azure OpenAI chat completion endpoints ("eastus" and "swedencentral"), the code uses an auto function invocation filter to inspect function calls during prompt processing. If a specific function (e.g., "GetEyeColor") is requested, the filter reroutes the request to a different deployment ("eastus"). This is particularly useful when semantic caching is enabled, ensuring that function execution utilizes the correct deployment as needed. The code also sets up a logging HTTP handler for diagnostics and includes test assertions and a sample prompt invocation, all within the xUnit testing framework.
