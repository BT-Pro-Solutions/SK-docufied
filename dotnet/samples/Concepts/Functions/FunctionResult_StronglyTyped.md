...
This C# code demonstrates how to obtain and handle results from a semantic kernel (Microsoft.SemanticKernel) using strong typing. It features a unit test (`FunctionResult_StronglyTyped`) that uses an OpenAI chat model to generate JSON data for test companies, parses the result into custom C# types, and extends the kernel's function result to include execution time and token usage statistics. Helper classes are included within the same file for JSON parsing and structuring, as well as for tokens accounting. The code highlights best practices for serialization, typing, and extension of function results when interfacing with language models.
