...
This C# example demonstrates how to use a vector database for semantic search using in-memory storage and Azure OpenAI embeddings. The core workflow involves creating an embedding generation service, ingesting structured "glossary" data into an in-memory vector store after embedding their definitions, and then performing semantic searches using both simple and filtered queries. The code outlines how to create a vectorized collection, generate embeddings asynchronously, upsert data, and run vector similarity searches with optional pre-filtering for fields marked as filterable. It also illustrates the use of record and data attributes for automatic vector store schema assignment.
