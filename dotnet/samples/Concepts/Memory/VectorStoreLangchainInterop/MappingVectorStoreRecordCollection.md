# Explanation
The `MappingVectorStoreRecordCollection` class is a decorator that allows converting vector store keys and record types between internal and "public" (user-visible) representations. This flexibility is useful if, for example, the underlying vector store library uses GUIDs for keys, but the application wants to work with string representations, or if different record types are needed at different layers. The class provides methods to map single and batch operations for CRUD and vectorized search by transforming keys and records back and forth via supplied mapping functions.  
**Note:** The class is currently disabled (behind `#if DISABLED`) as noted in comments, because efficient LINQ-based filtering now makes this kind of mapping more complex and uncommon.
