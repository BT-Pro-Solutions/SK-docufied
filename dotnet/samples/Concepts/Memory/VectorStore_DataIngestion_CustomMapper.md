...
This C# code file demonstrates how to ingest data into a Redis-based vector store using the Microsoft Semantic Kernel library, utilizing a custom mapper to bridge the gap between the application's data model and the storage model. The storage model is explicitly defined via a `VectorStoreRecordDefinition`, while the application works with a more generic data model featuring dictionaries for data and vectors.

Key aspects of the example include:

- Definition of the expected storage schema for glossary entries, including data and vector properties.
- Implementation of a custom mapper (`Mapper` class) handling conversion between the application's generic model and the storage schema.
- Use of Azure OpenAI embeddings to generate vector representations for glossary definitions.
- A custom vector store class that injects the custom mapper when the specific schema/type combination is used.
- Example code to ingest glossary entries, generate embeddings, upsert data into the vector store, and retrieve it back for validation.
- The use of a Docker-based Redis fixture for testing, demonstrating integration in a controlled environment.

This pattern is useful when the shape of the data in application logic significantly differs from the storage requirements of the vector database, requiring precise bidirectional mapping for reads and writes.
