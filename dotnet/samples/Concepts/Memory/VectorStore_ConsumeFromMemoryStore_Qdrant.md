# Explanation
This C# code demonstrates how to read vector data from a Qdrant data store with the newer VectorStore abstractions after creating that data with the legacy MemoryStore interfaces from Microsoft Semantic Kernel. It sets up a Qdrant Docker test environment, generates text embeddings using Azure OpenAI, creates a collection and adds data through the legacy memory interface, and then retrieves and prints records using the new VectorStore API. The included `VectorStoreRecord` type is designed to match the storage and schema used by `MemoryRecord` in `QdrantMemoryStore`, allowing seamless access to existing data with the updated API.
