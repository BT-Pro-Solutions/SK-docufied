# Explanation
This C# file demonstrates how to use the Microsoft Semantic Kernel with Gemini-based and Vertex-based Google AI embedding generation for semantic memory and similarity search in a .NET environment. The class `TextMemoryPlugin_GeminiEmbeddingGeneration` contains unit tests that show how to:

1. Configure Semantic Kernel with GoogleAI (Gemini) and VertexAI embedding and chat completion services.
2. Generate embeddings for text.
3. Save and retrieve memories (text snippets) using a volatile in-memory store. The memories are stored and recalled both directly (using `ISemanticTextMemory`) and via the `TextMemoryPlugin` imported into the kernel.
4. Perform a semantic similarity search to recall memories based on fuzzy intent matching using embeddings.
5. Use the memory recall within prompt functions, leveraging embedding-based memory for prompt augmentation.
6. Clean up the in-memory database collections after the sample runs.

Important context includes code comments on how to properly use bearer tokens with VertexAI in production (typically requiring more secure token management). The code is suitable as a reference and sample for developers wanting to integrate Google AI embeddings into Semantic Kernel workflows.
