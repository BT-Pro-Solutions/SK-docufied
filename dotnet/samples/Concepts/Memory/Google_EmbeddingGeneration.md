...
This C# code demonstrates how to use Microsoft's Semantic Kernel together with Google AI and Vertex AI services to generate text embeddings. The class provides unit tests (with xRetry for retries on HTTP failures) that show:
- How to generate embeddings via Google Vertex AI using OAuth2 authentication and a bearer token (custom embedding dimensions are not currently supported for Vertex AI).
- How to use Google AI for embedding generation with default model dimensions.
- How to specify custom embedding dimensions with Google AI.
Each test sets up a kernel with the relevant Google embedding generation service, requests embeddings for a given text, and outputs/validates the embeddings (including custom dimension checks). The code relies on proper configuration for API credentials and model IDs, and demonstrates Google OAuth token acquisition for Vertex AI.
