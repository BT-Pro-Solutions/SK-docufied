...
This C# example demonstrates how to use custom `JsonSerializerOptions` with the `TextMemoryPlugin` from Microsoft.SemanticKernel to control the serialization of recall results, especially when dealing with non-Latin scripts (like Thai and Bengali). By default, `JsonSerializer` escapes non-Latin characters, but you can override this by providing your own options. The code saves multilingual memories, performs recall twice (once with and once without custom options), and shows the effect on the output: the non-Latin scripts are escaped by default, and unescaped when custom options that include Thai and Bengali Unicode ranges are applied.
...
public class TextMemoryPlugin_RecallJsonSerializationWithOptions(ITestOutputHelper output) : BaseTest(output)
{
    ...
    public async Task RunAsync()
    {
        ...
        await textMemory.SaveInformationAsync("samples", "First example of some text in Thai and Bengali: วรรณยุกต์ চলিতভাষา", "test-record-1");
        ...
        // ["Second example of some text in Thai and Bengali: วรรณยุกต์ চলিতভাষา","First example of some text in Thai and Bengali: วรรณยุกต์ চলিতভাষা"]
