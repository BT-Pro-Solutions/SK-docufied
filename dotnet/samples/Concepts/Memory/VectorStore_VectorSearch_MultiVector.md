...
This C# code demonstrates how to perform vector search on a collection where each record contains multiple vector embeddings (e.g., a description embedding and a feature list embedding). It uses Azure OpenAI's embedding service to encode product data, stores everything in an in-memory vector store, and showcases searching based on different vector properties within each record. The code highlights:

1. Creating an in-memory vector store and defining a sample `Product` record with multiple embeddings.
2. Generating embeddings for product descriptions and feature lists using Azure OpenAI.
3. Upserting (inserting or updating) product records with these vectors into the store.
4. Performing vectorized searches on either the description or the feature list embedding property, showing the flexibility of searching within multi-vector records.

This workflow is useful for advanced semantic search scenarios where objects (like products) may have multiple semantic facets, each represented by its own vector embedding.
