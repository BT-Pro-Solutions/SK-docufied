...
This C# code demonstrates the interoperability between generic and custom data models when using a vector store with Microsoft's Semantic Kernel and Qdrant. The class `VectorStore_GenericDataModel_Interop` provides sample tests showing how you can upsert (insert or update) data into the vector store using one data model (generic or custom) and retrieve it with the other. It utilizes a Qdrant vector store running locally and leverages Azure OpenAI for embedding generation. 

The two main test methods are:
- `UpsertWithGenericRetrieveWithCustomAsync`: Upserts glossary entries using a generic data model with a defined schema, then retrieves those entries as instances of a strongly-typed custom class (`Glossary`).
- `UpsertWithCustomRetrieveWithGenericAsync`: Upserts glossary entries using the `Glossary` custom class, then retrieves those entries using the generic data model.

Supporting utility methods and model definitions are included to facilitate this demonstration.
