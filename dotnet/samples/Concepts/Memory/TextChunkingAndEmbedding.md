...
This C# code demonstrates how to split a long chat transcript into manageable chunks for embedding generation using Azure OpenAI's text embedding service. The process involves tokenizing and partitioning the input text, respecting both per-request token limits and API array size constraints. The code includes:

- Chunking the transcript based on tokens per line and per paragraph.
- Grouping these chunks into batches that fit within Azure OpenAI API's request constraints (maximum of 16 input items, under 8191 tokens for the "text-embedding-ada-002" model).
- Generating embeddings for each batch using the AzureOpenAITextEmbeddingGenerationService.
- Outputting progress and results to the console. 

The transcript used is a sample, realistic chat for demonstration purposes.
