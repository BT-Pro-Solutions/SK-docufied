...
This C# file demonstrates how to ingest and retrieve data using different types of vector stores: Redis, Qdrant, and an in-memory store. The example shows both dependency injection (DI) and manual instantiation approaches for setting up vector stores and embedding generation with Microsoft Semantic Kernel libraries. It handles differences in key types between the stores by using a generic key generator function. The main workflow includes registering services and stores, upserting sample glossary entries as embeddings, and retrieving them. The process can be tested against real Redis/Qdrant instances (via Docker), or a purely in-memory approach for local demonstration and unit testing.
