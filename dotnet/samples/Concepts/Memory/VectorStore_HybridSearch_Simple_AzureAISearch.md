# Explanation
This C# code file provides a simple example of how to ingest data into an Azure AI Search vector store and perform hybrid search operations. The process involves generating text embeddings with Azure OpenAI, storing glossary entries (each with text, category, and vector embedding fields) in the AzureAISearch vector store, and executing hybrid searches that combine vector similarity with keyword and filter-based queries. The sample demonstrates creating a glossary entity, generating its embedding, upserting records, and showing the result of different search scenarios—such as searching with keywords or applying pre-filters on the data. The code makes use of Semantic Kernel connectors for Azure services and employs modern C# syntax, test structure, and vector-enabled data modeling.
