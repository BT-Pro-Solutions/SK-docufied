# Explanation
This C# code provides two examples of using Semantic Kernel (SK) Semantic Memory to store and query data using natural language queries. The first example demonstrates semantic memory with Azure AI Search, which automatically handles semantic indexing. The second example shows how to build semantic memory using a custom embedding generator (OpenAI embeddings) and an in-memory (volatile) vector store. The program adds several GitHub file URLs and their descriptions to the semantic memory and demonstrates how to search this memory using natural language queries with relevance-ranked results. The code is organized for XUnit testing and illustrates both Azure and custom vector store setups.
