# Explanation
This C# code demonstrates how to contextually advertise functions to an AI model in a chat-based maze game scenario, leveraging the Microsoft Semantic Kernel. Depending on the game state (as reflected in chat history), only relevant functions are exposed to the AI model—for example, only advertising the "CreateMaze" function before the maze exists, and later allowing traps, treasures, and gameplay actions. The control logic prevents the AI from invoking functions out of order and maintains game progression. The script illustrates dynamic function advice using Semantic Kernel's function calling features, relevant for developers building adaptive AI dialog systems.
