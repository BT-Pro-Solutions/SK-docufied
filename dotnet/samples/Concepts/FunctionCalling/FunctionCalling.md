...
This C# file demonstrates various ways to configure and use function calling capabilities in Microsoft's Semantic Kernel (SK) library, particularly when interacting with OpenAI chat models. The examples show how to advertise kernel functions to the AI, control how and when functions get called (or not called), and manage invocation modes such as automatic or manual. The code covers key configuration options (`FunctionChoiceBehavior.Auto`, `.Required`, `.None`), both programmatically and via YAML template configs. It also illustrates advanced scenarios such as concurrent invocation, parallel calls, simulated function results, and disabling function calling entirely. The file provides self-contained, annotated test methods that act as usage recipes for developers integrating function calling with OpenAI chat completions.
