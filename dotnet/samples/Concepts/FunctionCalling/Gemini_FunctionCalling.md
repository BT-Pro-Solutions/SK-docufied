...
This C# code demonstrates two primary ways to invoke functions using Gemini Large Language Models (LLMs) via Semantic Kernel (SK) with both Google AI and Vertex AI. The code is designed as a set of integration-like tests using xRetry for test resilience. It walks the user through:

- **Automatic Function Invocation:** Functions called by the Gemini LLM are automatically executed through SK, with their results added to the chat history and considered by the LLM in generating its response.
- **Manual Function Invocation:** When the LLM requests a function, the function call is returned to the caller, who can decide how and when to execute it, handle exceptions, add results to the chat history, and continue the conversation.

The code configures SK to work with both Google AI and Vertex AI, sets up plugin functions (like getting the current UTC time and retrieving the weather for a city), and demonstrates both non-streaming and streaming chat completion. There are detailed comments throughout, as well as a commented-out section for interactive console usage.
