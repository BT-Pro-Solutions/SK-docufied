# Explanation
This C# code demonstrates how to use API Manifest plugins to call Microsoft Graph and NASA APIs within the Microsoft Semantic Kernel framework. The approach involves loading plugins described via API manifests, configuring authentication for each API (with MSAL for Graph and an API key for NASA), and invoking plugin functions. The code contains a test class with parameterized tests to validate plugin functionality and authentication. It also includes a utility class to apply bearer tokens to HTTP requests. To use this code, you must set up Azure AD application registrations and provide correct configuration settings.
