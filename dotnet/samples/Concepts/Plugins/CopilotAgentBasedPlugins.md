...
This C# code file demonstrates how to load and use Copilot Agent Plugins with Microsoft Semantic Kernel to interact with APIs such as Microsoft Graph and NASA APIs. The example covers the following key areas:

1. **Loading Copilot Agent Plugins:** Shows how to dynamically import plugins described by OpenAPI and manifest files.
2. **Authentication Configuration:** Handles authentication for different APIs, for example using MSAL for Microsoft Graph and an API key for NASA.
3. **Plugin Function Invocation:** Illustrates how to call specific functions from loaded plugins, passing in arguments and handling the results.
4. **Schema Trimming:** Includes logic to trim unnecessary properties from JSON schemas for certain strict API calls.
5. **Parameterized Testing:** Uses xUnit-style parameterized tests to demonstrate usage of multiple plugins and API functions.

To run these examples, the user must properly configure Microsoft Graph API credentials as explained in the comments section. The code makes use of various Semantic Kernel extension methods for plugin importation, and accommodates differences between APIs in terms of authentication and schema requirements.
