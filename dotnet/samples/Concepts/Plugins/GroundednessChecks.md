# Explanation
This C# code file provides tests for checking "groundedness" of generated content using Microsoft Semantic Kernel and OpenAI models. It defines a test class (`GroundednessChecks`) that demonstrates entity extraction, groundedness/reference checking, and removal (excision) of ungrounded entities from generated summaries. There are two major test methods:

1. **GroundednessCheckingAsync**: 
   - Initializes a Semantic Kernel instance with Azure OpenAI.
   - Loads sample plugins for summarization and grounding.
   - Extracts entities from a summary, checks if those entities are grounded in a given text, and excises (removes) ungrounded entities from the summary.
   - Prints each step's output for demonstration (extract entities, reference check, excise entities).

2. **PlanningWithGroundednessAsync**: 
   - Uses a `HandlebarsPlanner` to chain these operations using a high-level prompt (summarize, extract entities, check grounding, excise ungrounded).
   - Shows how planning works for orchestrating multiple plugins.

The tests use reference sample texts and demonstrate expected outputs in the final comment. This is a good illustration of automated groundedness assurance for generative AI systems.
