# Explanation
This C# code demonstrates how to set up and stream messages in a multi-agent chat environment using Microsoft Semantic Kernel. Specifically, it showcases communication between two agents: one leveraging chat completion (simulating an art director/Reviewer) and the other working as an OpenAI Assistant (simulating a copywriter). The `AgentGroupChat` class orchestrates the interaction with a custom termination strategy that halts the session when the reviewer "approves" the copy, or when a maximum number of turns is reached. The code also prints the streamed chat messages and displays the full conversation history at the end. This example is suitable for scenarios where two specialized AI agents collaborate and respond interactively to a creative task.
