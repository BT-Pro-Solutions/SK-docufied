...
This C# code file demonstrates how to use the AzureAIAgent in a streaming context with Azure AI, via the Microsoft Semantic Kernel. It contains several test scenarios showing how to:

- Create and interact with streaming agents that use Azure OpenAI,
- Use plugins (like the `MenuPlugin`) to extend agent functionality,
- Enable and stream responses from a code interpreter tool integrated with the agent,
- Stream and display chat messages as they are generated, including handling different message types (such as assistant responses and code interpreter outputs).

The class uses asynchronous tasks and xUnit `Fact` attributes for test methods. It features helper methods for agent invocation and chat history display. The `MenuPlugin` class is defined for demonstration and exposes menu specials and prices.

The file is suitable for developers looking to understand the mechanics of streaming agent conversations with Azure AI, agent plugins, and code interpreter tools in conversational scenarios.
