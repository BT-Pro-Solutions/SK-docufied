# Explanation
This code demonstrates how to use previous-generation Semantic Kernel APIs to generate images from text using OpenAI's DALL-E and Azure OpenAI's DALL-E 3 services. It includes tests illustrating the workflow for both services, showing how to use ITextToImageService to generate images from descriptive prompts, and how to guide a chat bot (IChatCompletionService) to respond with image descriptions rather than text, then generate corresponding images.

The first test (`OpenAIDallEAsync`) uses OpenAI's DALL-E 2 to create images from text descriptions, and orchestrates a conversation where the bot replies exclusively with image descriptions, which are then turned into image URLs.

The second test (`AzureOpenAIDallEAsync`, skipped by default) follows a similar workflow but utilizes Azure OpenAI's DALL-E 3, including a resiliency handler configuration for HTTP requests.

Both methods print prompts, image URLs, and the corresponding descriptions, simulating a chat where the user receives only images as bot responses.
