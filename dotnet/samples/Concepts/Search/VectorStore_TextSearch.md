...
This C# code demonstrates how to create, populate, and perform text searches over an in-memory vector store using Microsoft's Semantic Kernel and Vector Data libraries. The `VectorStore_TextSearch` test class illustrates:

- How to generate text embeddings using an OpenAI service.
- How to create a record collection in an `InMemoryVectorStore`, vectorize text entries, and store them as records with embeddings.
- How to perform and display various search operations (returning results as strings, structured results, and custom data models).
- Utilizes wrapper classes to handle vectorization of search queries and custom data model annotation for effortless storage and retrieval.

This example is useful for developers learning how to set up semantic search using vector databases and integrating AI-powered embeddings into .NET applications.
