...
This C# file demonstrates how to use the `BingTextSearch` class from the Microsoft Semantic Kernel library to perform text searches via the Bing web search API. It includes a series of test methods that show:
- How to instantiate and use `BingTextSearch` to search and retrieve various types of search results (as plain strings, as `TextSearchResult` objects, and as full Bing web page details).
- How to configure and use a custom string mapper for serializing search results.
- How to apply site-specific filters to web searches.
- Logging of HTTP requests/responses for diagnostics.

The code is suitable as a reference or test suite for developers integrating Bing search via the Semantic Kernel plugins.
