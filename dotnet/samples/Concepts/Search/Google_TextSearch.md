...
This C# code demonstrates how to use the `GoogleTextSearch` class from the `Microsoft.SemanticKernel.Plugins.Web.Google` namespace to perform Google-powered text searches. It includes several test cases showing different ways to use the search component:
- Basic search for string results, structured result objects (`TextSearchResult`), or the raw Google API result objects.
- Using a custom string mapper to serialize results as JSON.
- Filtering results by site using the `siteSearch` option.
The code also includes helper classes for logging HTTP requests and responses during tests, making it easier to debug and verify API interactions. It is intended for use in an automated test environment, using the xUnit test framework (`[Fact]` attributes).
