...
This C# code file demonstrates how to use the `TavilyTextSearch` class from the Microsoft Semantic Kernel library for performing web text searches using the Tavily Web Search API. It provides a series of example unit tests utilizing the xUnit testing framework (`[Fact]` attributes). The key usages shown include:

- Instantiating and configuring the `TavilyTextSearch` class with custom options such as logging, including raw content, images, image descriptions, and answers in the search.
- Performing searches and processing results in different formats: plain string, `TextSearchResult`, and the more detailed `TavilySearchResult`.
- Customizing result serialization using a custom string mapper.
- Applying a domain filter to the search using `TextSearchFilter`.
- The file relies on various Semantic Kernel and Tavily namespaces, as well as a testing infrastructure (such as `ITestOutputHelper`), presumed to be part of a test suite.
- Each example uses asynchronous patterns and demonstrates consumption of paged/streamed search results.
