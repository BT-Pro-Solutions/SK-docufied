...
This C# code demonstrates how to use the Microsoft Semantic Kernel SDK to integrate Bing and Google Search plugins for retrieving current information with AI prompts. The `BingAndGooglePlugins` class provides illustrative examples of how to connect to both Bing and Google search engines, import them as plugins into a `Kernel` (Semantic Kernel's central orchestrator), and use these plugins to fetch live data in response to user queries. The code includes two main example scenarios:

- Example 1 (`Example1Async`): Runs a search plugin (Bing or Google) to answer a direct question.
- Example 2 (`Example2Async`): Uses a semantic function (AI prompt) that decides if external data from search (Bing) is needed to answer multi-part, up-to-date questions. If so, it fetches the needed information via the search engine, injects it into the prompt, and re-evaluates the answer.

Credentials for OpenAI, Bing, and Google are required and referenced from a configuration. The code uses xUnit's `[Fact]` attribute for test purposes, but the example is skipped unless credentials are properly set up.
