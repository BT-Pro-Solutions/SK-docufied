# Explanation
This C# code demonstrates how to use a vector store (for similarity search and retrieval), where the underlying key type is not a string, specifically with Microsoft's Qdrant connector. The example switches from a string-keyed approach to one using an unsigned long (`ulong`) as the key type, leveraging a decorator class (`MappingVectorStoreRecordCollection`) to map between string and non-string key types transparently.

Key features of the code:
- Shows how to use the same client code for vector stores with different key types (string or non-string) without significant duplication, thanks to the mapping decorator.
- Provides an example test method (`UseAQdrantVectorStoreAsync`) which ingests data and runs a search using the Qdrant vector store.
- Defines a `UlongGlossary` class for records keyed by ulong and demonstrates conversion between string and ulong.
- Demonstrates how to create a mapping decorator to enable seamless conversion of keys and records between public-facing API contracts and the actual store implementation.

**Note:** This code block is conditionally compiled out (using `#if DISABLED_FOR_NOW`) and requires Qdrant server to be running for actual execution.
