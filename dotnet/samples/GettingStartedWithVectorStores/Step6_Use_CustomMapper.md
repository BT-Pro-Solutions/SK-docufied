...
This C# code demonstrates how to use a custom record mapper with an Azure AI Search vector store in a .NET application. The example shows how to define a custom data model (`ComplexGlossary`) that differs from the storage schema, and how to implement a custom mapper (`CustomMapper`) to translate between the application model and the underlying storage schema (represented as `JsonObject`). The code walks through defining the vector store schema, creating the collection if it doesn't exist, upserting a custom object, and performing a semantic (vectorized) search.

Key points:

- Shows how to separate application data models from storage schemas using custom mapping logic.
- Illustrates the use of Azure.AI.Search and Semantic Kernel vector data abstractions.
- Includes test logic for upserting and querying entities, showcasing the embedding process and search result processing.
